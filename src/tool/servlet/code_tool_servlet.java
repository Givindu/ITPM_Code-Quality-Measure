...
/**
 * Servlet implementation class code_tool_servlet
 */
@WebServlet("/code_tool_servlet")
public class code_tool_servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public code_tool_servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String code = request.getParameter("test_code");
		String code = request.getParameter("Inheritance");
		String code = request.getParameter("ControlStructure");
		String code = request.getParameter("Coupeling");
		HttpSession session = request.getSession();
		session.setAttribute("test_code",code);
		session.setAttribute("Inheritance",code);
		session.setAttribute("ControlStructure",code);
		session.setAttribute("Coupeling",code);
		...
		if(type.equals("size")) {
			...
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/test_results.jsp");
			...
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/all_results.jsp");
			dispatcher.forward(request, response);
		}
